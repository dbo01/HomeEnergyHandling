...
void update_oled()
{
	...
	case 1:
	{
		update_current(current_sensor, maxCurr, MinCurr);
...
void update_network()
{
	...
	// Draw the title
	sd1306_draw_string(OLED_TITLE_X, OLED_TITLE_Y, " Connecting", FONT_SIZE_TITLE, white_pixel);

	// Draw a label at line 1
	sd1306_draw_string(OLED_LINE_1_X, OLED_LINE_1_Y, str_SSID, FONT_SIZE_LINE, white_pixel);
	// Draw SSID string
	sd1306_draw_string(sizeof(str_SSID) * 6, OLED_LINE_1_Y, WiFiInfo.ssid, FONT_SIZE_LINE, white_pixel);
	...
	intToStr(WiFiInfo.frequencyMHz, string_data, 1);
	...
	channel = get_channel(WiFiInfo.frequencyMHz);
	...
	intToStr(WiFiInfo.signalRssi, string_data, 1);

	strcpy(string_data, "%d");
	snprintf(string_data, 10, aux_data_str, WiFiInfo.signalRssi);
	...
	// Draw a label at line 4
	sd1306_draw_string(OLED_LINE_4_X, OLED_LINE_4_Y, "Srv:", FONT_SIZE_LINE, white_pixel);

	// Draw RSSI value
	sd1306_draw_string(sizeof("Srv:") * 6, OLED_LINE_4_Y, SonoffSrvState, FONT_SIZE_LINE, white_pixel);
...
void update_current(float x, float y, float z)
{
	...
	// Draw the title
	sd1306_draw_string(OLED_TITLE_X, OLED_TITLE_Y, "   PRAD", FONT_SIZE_TITLE, white_pixel);
	...
	// Draw the units of x
	sd1306_draw_string(sizeof(str_light) * 10 + (get_str_size(string_data) + 1) * 10, OLEDA_LINE_1_Y, "[A]", FONT_SIZE_LINEA, white_pixel);

	// Convert y value to string
	ftoa(y, string_data, 2);

	// Draw a label at line 2
	sd1306_draw_string(OLEDA_LINE_2_X, OLEDA_LINE_2_Y, str_tbd1, FONT_SIZE_LINEA, white_pixel);
	// Draw the value of y
	sd1306_draw_string(sizeof(str_tbd1) * 10, OLEDA_LINE_2_Y, string_data, FONT_SIZE_LINEA, white_pixel);
	// Draw the units of y
